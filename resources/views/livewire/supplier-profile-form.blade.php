<div class="bg-white p-8 rounded-2xl shadow-xl border border-gray-100">
    ...
    <form wire:submit.prevent="save" class="space-y-6">
        <div>
            ...
            @error('business_name')
                <span class="text-red-500 text-xs mt-1 italic">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="p-4 bg-gray-50 rounded-lg border-2 border-dashed border-gray-200 text-center">
            ...
            @endif
            @error('license_proof')
                <span class="text-red-500 text-xs mt-1 italic block">{{ $message }}</span>
            @enderror
        </div>

        <button type="submit" wire:loading.attr="disabled" wire:target="save, license_proof"
            class="w-full flex justify-center py-3 px-4 border border-transparent rounded-xl shadow-sm text-sm font-bold text-white bg-green-600 hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed ...">

            <span wire:loading.remove wire:target="save">Submit Profile for Approval</span>
