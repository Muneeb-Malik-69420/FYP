<div class="h-full py-4 lg:pl-8 lg:border-l lg:border-gray-200 flex flex-col">
    <div class="bg-white border border-gray-200 flex flex-col h-full rounded-xl overflow-hidden">

        {{-- ── Header ── --}}
        <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100">
            <div class="flex items-center gap-2.5">
                <div wire:key="basket-icon-{{ $itemCount }}" class="relative animate-pop">
                    <i class="fas fa-shopping-cart text-[13px] text-gray-900"></i>
                    @if($itemCount > 0)
                        <span class="absolute -top-1.5 -right-2 bg-black text-white text-[7px] font-black
        ...
        <div class="flex-grow overflow-y-auto custom-scrollbar px-5 py-4 space-y-3">
            @forelse($cart as $id => $item)
                <div wire:key="basket-item-{{ $id }}"
                    ...
                    {{-- Thumbnail --}}
                    <div class="w-11 h-11 rounded-lg bg-gray-100 overflow-hidden border border-gray-200 shrink-0">
                        @if(!empty($item['image']))
                        ...
                        @else
                            <div class="w-full h-full flex items-center justify-center bg-gray-100">
                                <i class="fas fa-leaf text-gray-300 text-sm"></i>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="flex-grow min-w-0">
                        ...
                        <div class="flex justify-between items-start gap-1">
                            <h4 class="text-[9px] font-black text-gray-900 uppercase tracking-wide truncate leading-tight">
                                {{ $item['name'] }}
                            </h4>
                        ...
                        <div class="flex justify-between items-center mt-2">
                            <div class="flex items-center bg-white border border-gray-200 rounded-full overflow-hidden">
                    ...
                    <div class="relative w-20 h-20 mb-6">
                        ...
                        <div class="relative w-full h-full flex items-center justify-center">
                            <svg viewBox="0 0 80 80" fill="none" class="w-14 h-14 opacity-20">
                                <circle cx="40" cy="40" r="36" stroke="#111" stroke-width="2"/>
                                <path d="M25 30h4l5 18h12l4-12H30" stroke="#111" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                                <circle cx="36" cy="52" r="2" fill="#111"/>
                                <circle cx="46" cy="52" r="2" fill="#111"/>
                            </svg>
                        </div>
                    </div>
        ...
        <div class="px-5 pb-5 pt-3 border-t border-gray-100 space-y-3 mt-auto">
            ...
            {{-- Savings row --}}
            @if($savings > 0)
                <div class="flex justify-between items-center bg-gray-50 border border-gray-100 rounded-lg px-3 py-2">
                    <div class="flex items-center gap-1.5">
                        <i class="fas fa-tag text-[#52c234] text-[9px]"></i>
                        <span class="text-[9px] font-black uppercase tracking-widest text-gray-500">You Save</span>
                    </div>
                    <span class="text-[10px] font-black text-[#52c234] tabular-nums">
            ...
            <button wire:click="processCheckout"
                    ...
                    class="w-full bg-black hover:bg-[#52c234] text-white py-3.5 text-[10px] font-black
                           uppercase tracking-[0.25em] rounded-lg transition-all duration-300
