<div
    class="flex w-full items-center rounded-xl bg-white border border-gray-200 shadow-sm focus-within:ring-2 focus-within:ring-[#52c234]/20 transition-all duration-300">
    ...
    <div x-data="{ open: false }" class="relative border-r border-gray-100 bg-gray-50 min-w-[150px] rounded-l-xl">
        <button @click="open = !open" type="button" class="flex items-center gap-2 px-4 py-3 w-full group">
            ...
            <i class="fas fa-chevron-down text-[8px] text-gray-400 ml-auto transition-transform"
        ...
        <div x-show="open" @click.away="open = false" x-transition x-cloak
            ...
            <p class="text-[9px] font-black text-gray-400 uppercase tracking-widest p-2 border-b border-gray-50 mb-1">
                Available Cities</p>

            <div class="max-h-60 overflow-y-auto">
                @forelse($activeCities as $c)
                    <button type="button" wire:click.stop="selectCity('{{ $c->name }}')" @click="open = false"
                ...
                @empty
                    <div class="px-3 py-4 text-center">
                        <p class="text-[10px] text-gray-400 font-medium italic">No active cities found.</p>
                    </div>
                @endforelse
            </div>
        </div>
    ...
    <div class="relative flex-1">
        ...
        <input type="text" wire:model.live.debounce.400ms="searchQuery"
            placeholder="Search for 'Pizza' or 'Deals' in {{ $city }}..."
