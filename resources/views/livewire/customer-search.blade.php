<div class="flex w-full items-center rounded-xl bg-white border border-gray-200 shadow-sm focus-within:ring-2 focus-within:ring-[#52c234]/20 transition-all duration-300">
    ...
    <div x-data="{ open: false }" class="relative border-r border-gray-100 bg-gray-50 min-w-[150px] rounded-l-xl">
        ...
        <div x-show="open" @click.away="open = false" x-transition x-cloak 
            ...
            <div class="max-h-60 overflow-y-auto">
                @forelse($cities as $c)
                    <button wire:click="selectCity('{{ $c->name }}')" @click="open = false"
                            class="w-full text-left px-3 py-2.5 text-[11px] font-bold rounded-lg transition-colors flex items-center justify-between {{ $city == $c->name ? 'text-[#52c234] bg-green-50' : 'text-gray-600 hover:bg-gray-50' }}">
                        {{ strtoupper($c->name) }}
                        @if($city == $c->name)
                ...
                @empty
                    <div class="px-3 py-4 text-center">
                        <p class="text-[10px] text-gray-400 font-medium italic">No cities with active restaurants yet.</p>
                    </div>
                @endforelse
            </div>
        </div>
    ...
    <div class="relative flex-1">
        ...
        <input 
            ...
            placeholder="Search for 'Pizza', 'Restaurants' or 'Deals' in {{ $city }}..."
