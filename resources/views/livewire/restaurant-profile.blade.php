<div class="animate-fadeIn bg-white overflow-hidden font-sans">
            ...
            <div class="max-w-7xl mx-auto px-6">
                <nav class="flex items-center gap-3 text-white/50">
                    <button wire:click="closeRestaurant"
                        class="text-[9px] font-black uppercase tracking-[0.25em] hover:text-white transition-colors duration-200">
                        Explore
                    </button>
                ...
                <div class="flex items-end justify-between gap-6">
                    <div class="text-white max-w-3xl">
                        ...
                        <div class="flex flex-wrap items-center gap-2">
                            {{-- Eco-Partner — green, it's a certification --}}
                            <span class="inline-flex items-center gap-1.5 bg-[#000000] px-3 py-1 rounded-sm">
                                <svg class="w-2.5 h-2.5 text-white" fill="currentColor" viewBox="0 0 16 16">
                                    <path d="M8 .5a7.5 7.5 0 100 15A7.5 7.5 0 008 .5zm0 2a5.5 5.5 0 110 11A5.5 5.5 0 018 2.5z" />
                                </svg>
                            ...
                            {{-- Location --}}
                            <span class="inline-flex items-center gap-1.5 bg-white/10 backdrop-blur-sm border border-white/20 px-3 py-1 rounded-sm">
                                <i class="fas fa-map-marker-alt text-white/70 text-[9px]"></i>
    ...
    <div class="sticky top-[56px] z-40 bg-white/95 backdrop-blur-md border-b border-gray-200/80 shadow-sm">
        <div class="max-w-7xl mx-auto px-6 py-2.5 flex items-center gap-6 w-full">
            ...
            <div class="relative w-72 shrink-0 group">
                ...
                <input wire:model.live.debounce.300ms="search"
                    ...
                    class="w-full bg-gray-100/80 border border-gray-200 rounded-md py-2 pl-9 pr-8 text-[10px] font-black
                           tracking-[0.15em] text-gray-800 placeholder-gray-400 focus:ring-0 focus:border-black
            ...
            <div class="shrink-0 hidden md:flex items-center gap-1.5 text-[9px] font-black text-gray-400 uppercase tracking-widest">
                <span class="bg-black text-white px-2 py-0.5 rounded-full">{{ $foodItems->count() }}</span>
                <span>deals</span>
            </div>
        </div>
    </div>
    ...
    <div class="bg-gray-50">
        <div class="max-w-7xl mx-auto px-6">
            <div class="flex flex-col lg:flex-row items-start">
                ...
                <div class="w-full lg:flex-grow lg:pr-10 pt-8 pb-24 lg:h-[calc(100vh-130px)] lg:overflow-y-auto custom-scrollbar">
                    ...
                    <div wire:loading wire:target="search, setCategory" class="w-full">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 lg:gap-6 animate-pulse">
                            @for($i = 0; $i < 4; $i++)
                                <div class="bg-white rounded-2xl h-[180px] border border-gray-100"></div>
                            @endfor
                        </div>
                    </div>
                        ...
                        @forelse($foodItems as $item)
                            ...
                            <div wire:key="deal-{{ $item->id }}"
                                 class="group bg-white rounded-2xl border border-gray-100 p-4 flex justify-between min-h-[180px]
                                        ...
                                        {{ $isSoldOut ? 'opacity-60 cursor-not-allowed' : 'hover:shadow-xl hover:border-gray-200 hover:-translate-y-1.5 active:scale-[0.98] cursor-pointer' }}">
                                ...
                                <div class="flex flex-col justify-between py-1 pr-2 flex-1 min-w-0">
                                    <div>
                                        <div class="flex flex-wrap gap-1.5 mb-2">
                                            ...
                                            @elseif($percentage > 0)
                                                <span class="bg-black text-white text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded">
                                                    {{ $percentage }}% OFF
                                                </span>
                                            @endif
                                        ...
                                        <h3 class="text-gray-900 font-bold text-base leading-tight mb-1 truncate">
                                            {{ $item->item_name }}
                                        </h3>
                                        <p class="text-gray-400 font-normal text-xs line-clamp-2 leading-snug italic">
                                            {{ $item->description }}
                                        </p>
                                    </div>
                                    ...
                                    <div class="mt-3">
                                        @if(!$isLowStock && !$isSoldOut)
                                            <p class="text-[11px] font-semibold uppercase tracking-tight text-gray-400 mb-1">
                                                <i class="fas fa-layer-group mr-1"></i> {{ $item->quantity }} available
                                            </p>
                                        @endif

                                        <div class="flex items-center gap-2">
                                            <span class="text-lg font-extrabold text-gray-900 tracking-tighter">
                                                Rs.&nbsp;{{ number_format($item->discounted_price) }}
                                            </span>
                                            @if(!$isSoldOut && $item->original_price > $item->discounted_price)
                                                <span class="text-[11px] text-gray-300 font-medium line-through">
                                ...
                                <div class="relative flex-shrink-0 w-28 h-28 self-center overflow-hidden rounded-xl bg-gray-50 border border-gray-100">
                                    <img src="{{ $imageSrc }}"
                                         alt="{{ $item->item_name }}"
                                         loading="lazy"
                                    ...
                                    @else
                                        <button
                                            ...
                                            <div wire:loading wire:target="addToBasket({{ $item->id }})"
                                                 class="w-3.5 h-3.5 border-2 border-gray-900 border-t-transparent rounded-full animate-spin"></div>
                                        </button>
                                    @endif
                                </div>
                            </div>
                ...
                {{-- RIGHT: Basket --}}
                <aside class="hidden lg:block lg:w-[380px] lg:h-[calc(100vh-130px)] lg:sticky lg:top-[130px]">
    ...
    <div class="lg:hidden fixed bottom-6 left-1/2 -translate-x-1/2 z-50"
        ...
        <button class="bg-black text-white px-6 py-3.5 rounded-full shadow-2xl flex items-center gap-3
            ...
            <span x-show="count > 0"
                  class="bg-white text-black text-[9px] font-black w-5 h-5 rounded-full flex items-center justify-center"
                  x-text="count"></span>
        </button>
    </div>
    ...
    <div
        ...
        style="display: none;">
        <div class="bg-black text-white px-5 py-3 rounded-full shadow-2xl flex items-center gap-3 border border-white/10 pointer-events-auto max-w-xs">
            <div :class="type === 'success' ? 'bg-white' : 'bg-red-500'"
                 class="rounded-full w-5 h-5 flex items-center justify-center shrink-0">
                <i :class="type === 'success' ? 'fa-check text-black' : 'fa-times text-white'" class="fas text-[9px]"></i>
            </div>
