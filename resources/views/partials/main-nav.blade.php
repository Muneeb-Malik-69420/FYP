<nav id="navbar"
    ...
    <div class="flex items-center gap-0.5">
        ...
        {{-- CART (public — guests + auth) --}}
        <button onclick="window.dispatchEvent(new CustomEvent('open-cart'))"
                class="{{ $iconClass }}"
                x-data="{ count: {{ array_sum(array_column(session()->get('cart', []), 'quantity')) }} }"
                x-on:cart-updated.window="count = $event.detail.count">
            <i class="fas fa-shopping-cart"></i>
            <span x-show="count > 0"
                  x-text="count > 9 ? '9+' : count"
                  x-cloak
                  class="absolute -top-0.5 -right-0.5 bg-gray-800 text-white text-[8px] font-black h-4 w-4 rounded-full flex items-center justify-center leading-none">
            </span>
        </button>
            ...
            <div class="relative ml-1"
                ...
                <button @click="open=!open"
                    ...
                    <div class="w-8 h-8 rounded-full overflow-hidden border-2 border-transparent group-hover:border-black transition duration-200">
                        <img src="https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->username ?? auth()->user()->name ?? 'U') }}&background=111111&color=fff"
    ...
    <div x-show="mobileOpen"
        ...
        <div class="flex-1 px-6 py-6 space-y-1">
            ...
            @auth
                ...
                <button onclick="window.dispatchEvent(new CustomEvent('open-favourites')); mobileOpen=false"
                        class="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest text-gray-800 hover:bg-gray-50 transition">
                    <i class="far fa-heart text-gray-400 w-4 text-center"></i> Favourites
                </button>
                <button onclick="window.dispatchEvent(new CustomEvent('open-cart')); mobileOpen=false"
                        class="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest text-gray-800 hover:bg-gray-50 transition">
                    <i class="fas fa-shopping-cart text-gray-400 w-4 text-center"></i> Basket
                </button>
