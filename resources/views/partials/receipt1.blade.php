<div class="min-h-screen bg-white flex flex-col items-center justify-start py-2 px-4 animate-fadeIn font-sans">
    ...
    <div class="flex flex-col items-center text-center mb-10">
        ...
        <p class="text-[10px] font-black uppercase tracking-widest text-primary mb-1">Success</p>
        <h1 class="text-4xl md:text-5xl font-black text-gray-900 tracking-tight uppercase leading-[0.9]">Order Confirmed!</h1>
        <p class="text-gray-400 mt-3 text-xs font-medium tracking-wide max-w-sm">
    ...
    <div class="w-full max-w-xl">
        <div class="bg-cardcolor rounded-sm overflow-hidden">

            {{-- Receipt header --}}
            <div class="px-8 py-6 border-b border-gray-200 flex items-center justify-between">
                <div>
                    <p class="text-[10px] font-black uppercase tracking-widest text-primary mb-1">Receipt</p>
                    <p class="text-xl font-black text-gray-900 uppercase tracking-tight">Order #{{ $order->id }}</p>
                </div>
            ...
            <div class="px-8 py-6 space-y-6">
                ...
                <div class="flex items-center justify-between">
                    <span class="inline-flex items-center gap-1.5 bg-black text-white text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-sm">
                        <i class="fas fa-circle text-[6px] text-primary"></i>
                        {{ ucfirst($order->status) }}
                    </span>
                    <div class="flex items-center gap-2 text-gray-400">
                        <i class="fas {{ $order->payment_method === 'card' ? 'fa-credit-card' : 'fa-money-bill-wave' }} text-xs"></i>
                ...
                {{-- Delivery info --}}
                <div class="bg-white rounded-sm p-5 border border-gray-200 space-y-2.5">
                        ...
                        @foreach($order->items as $item)
                            <div class="flex items-center justify-between gap-4">
                                <div class="flex items-center gap-3 min-w-0">
                                    <span class="shrink-0 w-6 h-6 bg-black text-white text-[9px] font-black rounded-sm flex items-center justify-center">
                                        {{ $item->quantity }}
                                    </span>
                ...
                <div class="relative -mx-8">
                    <div class="flex items-center">
                        <div class="w-5 h-5 bg-cardcolor border border-gray-200 -ml-2.5 rounded-full shrink-0"></div>
                        <div class="flex-1 border-t border-dashed border-gray-200 mx-1"></div>
                        <div class="w-5 h-5 bg-cardcolor border border-gray-200 -mr-2.5 rounded-full shrink-0"></div>
                    </div>
                </div>
            ...
            {{-- Footer --}}
            <div class="px-8 pb-8 flex flex-col items-center gap-4 border-t border-gray-200 pt-6">
                ...
                <button wire:click="goHome"
                        class="w-full bg-black hover:bg-primary second disabled:opacity-50
                               text-white py-4 rounded-sm font-black uppercase text-[11px] tracking-widest
                               transition-all duration-300 active:scale-[0.98] shadow-lg shadow-black/10
        ...
        <div class="flex items-center justify-center gap-2 mt-6">
            ...
            <span class="text-[10px] font-black uppercase tracking-widest text-gray-400">You're helping reduce food waste. Thank you!</span>
        </div>
    </div>
